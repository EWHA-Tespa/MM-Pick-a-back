...
class perceiver_io(nn.Module):
    def __init__(
        ...
        *,
        ...
        # caching setup
        get_cross_attn = lambda: PreNorm(latent_dim, MultiModalAttention(query_dim=latent_dim, image_context_dim=image_input_dim, text_context_dim=text_input_dim, heads=cross_heads, dim_head=cross_dim_head, dropout=attn_dropout), context_dim=None)
        get_self_attn = lambda: PreNorm(latent_dim, Attention(latent_dim, heads=latent_heads, dim_head=latent_dim_head, dropout=attn_dropout))
    ...
    def forward(self, data, mask=None, queries=None, return_embeddings=False, modality=None):
        ...
        # classification only 경로 - decoder 건너뜀
    ...
    def _reconstruct_classifiers(self):
        for dataset, num_classes in self.dataset2num_classes.items():
            self.classifiers.append(nl.SharableLinear(84, num_classes, bias=True, mask_init='1s', mask_scale=1e-2, threshold_fn='binarizer'))
        if self.classifiers:
    ...
    def _initialize_weights(self):
        for m in self.modules():
            if isinstance(m, nn.Conv2d):
                ...
                nn.init.constant_(m.bias, 0)
            elif isinstance(m, nn.Linear) or isinstance(m, nl.SharableLinear):
