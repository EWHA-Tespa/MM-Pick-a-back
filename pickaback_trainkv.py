...
def freeze_all_except_kv(model, modality):
    """KV 를 제외한 모든 파라미터 freeze"""
    for n, p in model.named_parameters():
        if ("to_kv_image" in n and modality == 'image') or \
           ("to_kv_text"  in n and modality == 'text'):
            p.requires_grad = True
        else:
            p.requires_grad = False

def kv_light_adapt_epoch(model, ref_model, dataloader,
                         modality, num_epochs=2, lr=1e-4):
    freeze_all_except_kv(model, modality)
    opt = torch.optim.AdamW(
            [p for p in model.parameters() if p.requires_grad], lr=lr)

    model.train();  ref_model.eval()
    for _ in range(num_epochs):
        for x, _ in dataloader:           # 한 epoch 전체 사용
            x = x.cuda(non_blocking=True)
            with torch.no_grad():
                z_ref = ref_model.encode_latent(x)
            z = model.encode_latent(x)
            loss = 0.5*(1 - torch.cosine_similarity(z, z_ref).mean())
...
for task_id in range(start_index, num_groups + 1):
    ...
    lr = 1e-2
    weight_decay = 4e-5
    load_folder = 'checkpoints_' + arch + '/baseline_scratch/' + arch + '/' + dataset_name  #model1이 task, model2가 target 
    ...
    with torch.no_grad():
        ...
        if manager.cuda:
            ...
            model_ref  = copy.deepcopy(manager.model)
            model2_ref = copy.deepcopy(manager2.model)
            ...
            kv_light_adapt_epoch(NewModel1, model_ref, train_loader2, target_modality, num_epochs=2, lr=1e-4)
            kv_light_adapt_epoch(NewModel2, model2_ref, train_loader, task_modality, num_epochs=2, lr=1e-4)
