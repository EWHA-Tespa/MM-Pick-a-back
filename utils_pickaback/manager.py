...
class Manager(object):
    ...
    def __init__(self, dataset_name, mode, network_width_multiplier, log_path, checkpoint_format, 
                 finetune_again, target_sparsity, initial_sparsity, pruning_frequency, weight_decay,
                 model, shared_layer_info, masks, train_loader, val_loader, begin_prune_step, end_prune_step):
        ...
        self.inference_dataset_idx = self.model.datasets.index(dataset_name) + 1 ##### Jinee ##### self.model.module.datasets.index(dataset_name) + 1
        self.pruner = SparsePruner(mode, dataset_name, network_width_multiplier, 
                                   finetune_again, target_sparsity, initial_sparsity, pruning_frequency, weight_decay,
                                   self.model, masks, begin_prune_step, end_prune_step, self.inference_dataset_idx)

        self.train_loader = train_loader
        self.val_loader   = val_loader

        if dataset_name == 'face_verification':
    ...
    def train(self, optimizers, epoch_idx, curr_lrs, curr_prune_step):
        ...
        with tqdm(total=len(self.train_loader),
            ...
            for batch_idx, (data, target) in enumerate(self.train_loader):
                ...
                num = data.size(0)
                if self.dataset_name != 'face_verification':
                ...
                if self.inference_dataset_idx == 1:
                    t.set_postfix({'loss': train_loss.avg.item(),
                                   ...
                                   'network_width_mpl': self.network_width_multiplier})
                else:
                    t.set_postfix({'loss': train_loss.avg.item(),
                                   'accuracy': '{:.2f}'.format(100. * train_accuracy.avg.item()),
                                   'lr': curr_lrs[0],
                ...
                for data, target in self.val_loader:
                    ...
                    if self.inference_dataset_idx == 1:
                        t.set_postfix({'loss': val_loss.avg.item(),
                                       ...
                                       'mpl': self.network_width_multiplier})
                    else:
                        t.set_postfix({'loss': val_loss.avg.item(),
                                       'accuracy': '{:.2f}'.format(100. * val_accuracy.avg.item()),
                                       'sparsity': self.pruner.calculate_sparsity(),
    ...
    def save_checkpoint(self, optimizers, epoch_idx, save_folder):
        """Saves model to file."""
        filepath = self.checkpoint_format.format(save_folder=save_folder, epoch=epoch_idx + 1)

        for name, module in self.model.named_modules():#.module.named_modules():
            if isinstance(module, nl.SharableConv2d) or isinstance(module, nl.SharableLinear):
                if module.bias is not None:
                    self.shared_layer_info[self.dataset_name][
        ...
        checkpoint = {
            'model_state_dict': self.model.state_dict(),#.module.state_dict(),
            'dataset_history': self.model.datasets,#.module.datasets,
            'dataset2num_classes': self.model.dataset2num_classes,#.module.dataset2num_classes,
    ...
    def load_checkpoint(self, optimizers, resume_from_epoch, save_folder):
        ...
        if resume_from_epoch > 0:
            filepath = self.checkpoint_format.format(save_folder=save_folder, epoch=resume_from_epoch)
            checkpoint = torch.load(filepath)
            checkpoint_keys = checkpoint.keys()
            state_dict = checkpoint['model_state_dict']
            curr_model_state_dict = self.model.state_dict()#.module.state_dict()
    ...
    def load_checkpoint_only_for_evaluate(self, resume_from_epoch, save_folder):
        ...
        if resume_from_epoch > 0:
            ...
            for name, module in self.model.named_modules():#.module.named_modules():
                if isinstance(module, nl.SharableConv2d) or isinstance(module, nl.SharableLinear):
                    if module.bias is not None:
                        module.bias = self.shared_layer_info[self.dataset_name]['bias'][name]

                elif isinstance(module, nn.BatchNorm2d):
                    module.running_mean = self.shared_layer_info[self.dataset_name][
                        'bn_layer_running_mean'][name]
                    module.running_var = self.shared_layer_info[self.dataset_name][
                        'bn_layer_running_var'][name]
                    module.weight = self.shared_layer_info[self.dataset_name][
                        'bn_layer_weight'][name]
                    module.bias = self.shared_layer_info[self.dataset_name][
                        'bn_layer_bias'][name]
